...
export type DiscountResult = { ok: true; message: string } | { ok: false; error: string };

/**
 * Set or clear a per-product discount.
 * - `percent`: 0–100. 0 clears the discount.
 * - `expiresAt`: ISO datetime or null. When in the past, storefront ignores the discount.
 */
export async function setDiscount(
  productId: string,
  percent: number,
  ...
  const exp = expiresAt && expiresAt.trim() !== "" ? new Date(expiresAt).toISOString() : null;
  ...
  try {
    ...
    const { error } = await db
      ...
      .update({
        discount_percent: p,
        is_on_sale: p > 0,
        discount_expires_at: p > 0 ? exp : null,
      })
      .eq("id", productId);
    if (error) return { ok: false, error: error.message };
  } catch (e) {
  ...
  revalidatePath("/shop");
  revalidatePath("/");
  return { ok: true, message: p > 0 ? `${p}% хямдрал тохируулагдлаа` : "Хямдрал хасагдлаа" };
}
